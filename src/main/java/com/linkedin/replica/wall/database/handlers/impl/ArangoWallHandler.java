...
public class ArangoWallHandler implements WallHandler {
	...
	/**
	 * FOR user in users filter user.userId == "10"
	 * 
	 * LET friendPosts = ( for friend in users filter friend.userId in
	 * user.friendsList for post in posts FILTER post.authorId == friend.userId
	 * SORT post.timestamp DESC Limit 0, 10 return MERGE( [
	 * 
	 * { author : { "authorName": concat(friend.firstName, " ",
	 * friend.lastName), "authorProfilePictureUrl" : friend.profilePictureUrl }
	 * }, {post : post} ] ) )
	 * 
	 * LET companiesPosts = ( for company in companies filter company.companyId
	 * in user.followedCompanies FOR post in posts FILTER post.authorId ==
	 * company.companyId and post.timestamp >= "24" SORT post.weight DESC Limit
	 * 0, 10 return MERGE( [
	 * 
	 * { author : { "authorName": company.companyName, "authorProfilePictureUrl"
	 * : company.profilePictureUrl } }, {post : post} ] ) )
	 * 
	...
	public List<Post> getNewsFeed(String userId, int limit) {
		...
		builder.append("{ author : { \"authorName\":  concat(friend.firstName, \" \", friend.lastName), ");
		...
		builder.append("Limit 0, @limit ");
		builder.append("return MERGE_RECURSIVE( ");
		builder.append("{ author : { \"authorName\": company.companyName, ");
		...
		cursor.forEachRemaining(postDocument -> {
			...
			System.out.println(new Gson().toJson(postDocument.getAttribute("results")));
		});
		return null;
	}
	...
	public String addBookmark(Bookmark bookmark) {
		...
		if (postId != null && getPost(postId) != null) {
			try {
				UserProfile user = arangoDB.db(dbName).collection(usersCollection)
						.getDocument(userId, UserProfile.class);
	...
	public UserProfile getUser(String userId) {
		...
		try {
			user = arangoDB.db(dbName).collection(usersCollection).getDocument(userId, UserProfile.class);
		} catch (ArangoDBException e) {
			...
			for (String key : args.keySet()) {
				if (!key.equals("postId") && !key.equals("authorId")) {
					...
					for (int i = 0; i < postFields.length; i++) {
						if (key.equals(postFields[i].getName())
								&& String.class.isAssignableFrom(postFields[i].getType())) {
							query += "\"" + args.get(key) + "\" ";
							isString = true;
							break;
						}
	...
	public List<Comment> getComments(String postId) {
		...
		try {
			...
			ArangoCursor<Comment> cursor = arangoDB.db(dbName).query(query, bindVars, null, Comment.class);
	...
	public Comment getComment(String commentId) {
		...
		try {
			comment = arangoDB.db(dbName).collection(commentsCollection).getDocument(commentId, Comment.class);
		} catch (ArangoDBException e) {
	...
	public String addComment(Comment comment) {
		...
		if (postId != null && getPost(postId) != null) {
			...
			try {
				DocumentCreateEntity commentDoc = arangoDB.db(dbName).collection(commentsCollection)
			...
			for (String key : args.keySet()) {
				if (!key.equals("commentId")) {
					...
					for (int i = 0; i < commentFields.length; i++) {
						if (key.equals(commentFields[i].getName())
								&& String.class.isAssignableFrom(commentFields[i].getType())) {
							query += "\"" + args.get(key) + "\" ";
							isString = true;
							break;
						}
	...
	public List<Reply> getReplies(String commentId) {
		...
		try {
			...
			ArangoCursor<Reply> cursor = arangoDB.db(dbName).query(query, bindVars, null, Reply.class);
			...
			for (String key : args.keySet()) {
				if (!key.equals("replyId")) {
					...
					for (int i = 0; i < replyFields.length; i++) {
						if (key.equals(replyFields[i].getName())
								&& String.class.isAssignableFrom(replyFields[i].getType())) {
							query += "\"" + args.get(key) + "\" ";
							isString = true;
							break;
						}
	...
	public Like getLike(String likeId) {
		Like like = null;
		try {
			like = arangoDB.db(dbName).collection(likesCollection).getDocument(likeId, Like.class);
		} catch (ArangoDBException e) {
	...
	public List<Like> getPostLikes(String postId) {
		...
		try {
			...
			ArangoCursor<Like> cursor = arangoDB.db(dbName).query(query, bindVars, null, Like.class);
	...
	public String addLike(Like like) {
		...
		if ((postId != null && getPost(postId) != null) || (commentId != null && getComment(commentId) != null)
				|| (replyId != null && getReply(replyId) != null)) {
			try {
	...
	public void getTopPosts() throws ParseException {
		try {
			...
			ArangoCursor<Post> cursor = arangoDB.db(dbName).query(query, null, null, Post.class);
			cursor.forEachRemaining(postDocument -> {
			});
		} catch (ArangoDBException e) {
